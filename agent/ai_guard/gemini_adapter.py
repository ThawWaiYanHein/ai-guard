...
def _candidate_classes() -> list[tuple[type, str, str, bool]]:
    ...
    for module_name, class_names in (
        ...
        ("google.genai.resources.models", ("Models", "AsyncModels")),
    ):
...
def patch_gemini(config: Config | None = None) -> None:
    global _PATCHED
    if _PATCHED:
        return
    ...
    _PATCHED = True
    log_event(config, action="patched", reason=f"patched {patched} Gemini resource classes")
