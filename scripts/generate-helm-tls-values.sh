...
OUT="${OUT:-charts/ai-guard/tls-values.generated.yaml}"
TMP_DIR="$(mktemp -d)"
...
cat > "$OUT" <<EOF
tls:
  crt: "$(base64 < "$TMP_DIR/tls.crt" | tr -d '\n')"
  key: "$(base64 < "$TMP_DIR/tls.key" | tr -d '\n')"
  caBundle: "$(base64 < "$TMP_DIR/ca.crt" | tr -d '\n')"
EOF

echo "Rendered $OUT for service $SERVICE in namespace $NAMESPACE"
