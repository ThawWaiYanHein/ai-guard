param(
    ...
    [string]$Secret = "ai-guard-webhook-tls",
    [string]$Manifest = "k8s/ai-guard-operator.yaml",
    [string]$Out = "k8s/ai-guard-operator.rendered.yaml"
)

$ErrorActionPreference = "Stop"
$tmp = New-Item -ItemType Directory -Path ([System.IO.Path]::Combine([System.IO.Path]::GetTempPath(), "ai-guard-certs-" + [guid]::NewGuid()))
...
"@
    ...
    (Get-Content -LiteralPath $Manifest -Raw).
        Replace('${CA_BUNDLE}', $caBundle).
        Replace('${TLS_CRT}', $tlsCrt).
        Replace('${TLS_KEY}', $tlsKey) | Set-Content -LiteralPath $Out
    Write-Host "Rendered $Out with CA bundle and TLS secret data for $Namespace/$Secret"
}
finally {
